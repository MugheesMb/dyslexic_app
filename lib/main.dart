...
import 'package:dyslexiaa/OnBoardingScreens/onboard_screen.dart';
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';

import 'Dygnosis/quizhome.dart';
import 'Dygnosis/quizmain.dart';
...
import 'MathActivites/sub7to11/subtraction_1_7to11.dart';
...
import 'OnBoardingScreens/first_onboarding.dart';
import 'OnBoardingScreens/second_onboarding.dart';
import 'ReadingActivity/letters_4to6.dart';
import 'ReadingActivity/reading_activity_screen.dart';
import 'ReadingActivity/words_4to6.dart';
...
import 'WritingActivites/writing_activity_screen.dart';
...
class MyApp extends StatelessWidget {
  ...
  Future<Widget> _getInitialScreen() async {
    ...
    if (!hasSeenOnboarding) {
      return OnBoardScreen(); // Use the proper combined onboarding screen
    } else if (user != null) {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        home: SplashScreenWrapper(getInitialScreen: _getInitialScreen),
        routes: {
          SplashScreen.routeName: (ctx) => SplashScreen(),
          AnimationWidget.routeName: (ctx) => AnimationWidget(),
          OnBoardScreen.routeName: (ctx) => OnBoardScreen(), // Use the combined screen
...
class SplashScreenWrapper extends StatefulWidget {
  final Future<Widget> Function() getInitialScreen;
  const SplashScreenWrapper({super.key, required this.getInitialScreen});
...
class _SplashScreenWrapperState extends State<SplashScreenWrapper> {
  ...
  void _navigate() async {
    // Show splash for 2 seconds for better visibility
    await Future.delayed(Duration(milliseconds: 2000));

    if (mounted) {
      Widget next = await widget.getInitialScreen();
